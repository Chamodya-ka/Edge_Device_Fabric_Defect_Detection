...
float* FeatureComputation::getFeatures(float* subGLCM,int gl,int rows, int cols,unsigned int subImgDim){
	...
	//cudaMemcpyAsync(subGLCM,h_sGLCM,N * gl * gl *4 * intsize,cudaMemcpyHostToDevice,stream[0]);
	//cudaMemcpyAsync(subGLCM,h_sGLCM_1,N * gl * gl *4 * intsize,cudaMemcpyHostToDevice,stream[1]);
	//cudaMemcpyAsync(subGLCM,h_sGLCM_2,N * gl * gl *4 * intsize,cudaMemcpyHostToDevice,stream[2]);
	//cudaMemcpyAsync(subGLCM,h_sGLCM_3,N * gl * gl *4 * intsize,cudaMemcpyHostToDevice,stream[3]);
	//cudaMemcpyAsync(subGLCM,h_sGLCM_4,N * gl * gl * 4 * intsize,cudaMemcpyHostToDevice,stream[4]);
	...
	ContrastFeature2<<<blocks,threads,0,stream[1]>>>(gl , d_inp_1 ,d_feat_2);
	EntropyFeature2<<<blocks,threads,0,stream[2]>>>(gl , d_inp_1 ,d_feat_3);
	HomogeneityFeature2<<<blocks,threads,0,stream[3]>>>(gl , d_inp_1 ,d_feat_4);
	CorrelationFeature2<<<blocks,threads,0,stream[4]>>>(gl , d_inp_1 ,d_feat_5,meanX_d,meanY_d,stddX_d,stddY_d);
